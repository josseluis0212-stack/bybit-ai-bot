...
import gevent
import time
import requests # Para keep-alive ping
import yaml
import os
...
def keep_alive_ping():
    """
    Mantiene vivo el servicio en Render Free Tier (evita auto-sleep por inactividad).
    Hace ping a su propia URL cada 10 minutos.
    """
    url = "https://bit-ia-nuevo.onrender.com" 
    print(f"⏰ Iniciando sistema Keep-Alive hacia: {url}")
    while True:
        try:
            response = requests.get(url)
            print(f"✅ Keep-Alive Ping: {response.status_code}")
        except Exception as e:
            print(f"❌ Error en Keep-Alive Ping: {e}")
        
        gevent.sleep(600) # 10 minutos

if __name__ == "__main__":
    # Iniciar el bucle del bot usando gevent (mejor para SocketIO)
    gevent.spawn(bot_loop)
    
    # Iniciar ping de auto-mantenimiento
    gevent.spawn(keep_alive_ping)
