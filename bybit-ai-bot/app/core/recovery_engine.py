        ...
        for sym, pos in exchange_map.items():
            if sym not in processed_symbols:
                try:
                    ...
                    logger.warning(f"🛡️ [RECOVERY] Colocando protecciones para huérfano {sym}: SL={sl_price:.4f}")
                    ...
                    try:
                        ...
                        logger.error(f"[RECOVERY] No se pudieron colocar TPs/SLs para huérfano {sym}, pero fue adoptado en memoria. Error: {e}")
                    
                    await self.engine.ws.subscribe_mark_price(sym)
                    adopted += 1
                except Exception as e:
                    logger.error(f"[RECOVERY] Falla total al intentar adoptar {sym}: {e}")
