...
class InstitutionalSMCStrategy:
    ...
    def __init__(self):
        ...
        self.min_adx = (
            22.0  # Filtro de tendencia: Solo operamos con tendencia clara (> 22)
        )
    ...
    def calculate_iss(self, candle_ob, impulse_candle, volume_ma):
        ...
        displacement_score = min(
            displacement_ratio * 35, 100
        )  # Más difícil llegar a 100
        ...
        if len(highs) >= 2:
            for i in range(len(highs)):
                for j in range(i + 1, len(highs)):
                    diff = (
                        abs(highs.iloc[i]["high"] - highs.iloc[j]["high"])
                        / highs.iloc[i]["high"]
                        * 100
                    )
                    if diff < threshold:
                        eqh.append(
                            {
                                "price": (highs.iloc[i]["high"] + highs.iloc[j]["high"])
                                / 2
                            }
                        )
        if len(lows) >= 2:
            for i in range(len(lows)):
                for j in range(i + 1, len(lows)):
                    diff = (
                        abs(lows.iloc[i]["low"] - lows.iloc[j]["low"])
                        / lows.iloc[i]["low"]
                        * 100
                    )
                    if diff < threshold:
                        eql.append(
                            {"price": (lows.iloc[i]["low"] + lows.iloc[j]["low"]) / 2}
                        )
        return eqh, eql
    ...
    def analyze(self, symbol: str, df: pd.DataFrame, df_htf: pd.DataFrame = None):
        ...
        df["high_fractal"] = (
            df["high"]
            .rolling(window=self.swing_period * 2 + 1, center=True)
            .apply(
        ...
        for i in range(len(df)):
            if df.iloc[i]["high_fractal"] == 1:
                last_highs.append(df.iloc[i]["high"])
            if df.iloc[i]["low_fractal"] == 1:
                last_lows.append(df.iloc[i]["low"])
                ...
                for j in range(i - 1, max(0, i - 15), -1):
                    if df.iloc[j]["close"] < df.iloc[j]["open"]:
                        ...
                        iss = self.calculate_iss(
                            df.iloc[j], df.iloc[i], df.iloc[i]["vol_ma"]
                        )
        ...
        if c_price < equilibrium and c_price > current["ema_200"]:
            for ob in reversed(bull_obs[-self.ob_limit :]):
                ...
                if current["low"] <= ob["high"] and c_price > ob["low"]:
                    ...
                    htf_ok = (
                        (htf_trend == "BULLISH") if settings.HTF_CONFLUENCE else True
                    )

                    if (
                        (liquidity_sweep or mss)
                        and is_discount
                        and is_undervalued
                        and htf_ok
                    ):
                        ...
                        entry_price = (
                            min(c_price, ote_705) if c_price > ote_705 else c_price
                        )

                        # SL SEGURO: 0.6 * ATR (más cerca)
                        sl = min(ob["low"], current["low"]) - (current["atr"] * 0.6)
        ...
        if c_price > equilibrium and c_price < current["ema_200"]:
            for ob in reversed(bear_obs[-self.ob_limit :]):
                ...
                if current["high"] >= ob["low"] and c_price < ob["high"]:
                    ...
                    # HTF Confluence Check
                    htf_ok = (
                        (htf_trend == "BEARISH") if settings.HTF_CONFLUENCE else True
                    )
                    ...
                    if (
                        ...
                        entry_price = (
                            max(c_price, ote_705) if c_price < ote_705 else c_price
                        )

                        # SL SEGURO: 0.6 * ATR (más cerca)
                        sl = max(ob["high"], current["high"]) + (current["atr"] * 0.6)
        ...
        for brk in breakers:
            if brk["type"] == "BULLISH_BREAKER":
                ...
                if current["low"] <= brk["price"] * 1.001 and c_price > brk["price"]:
                    sl = current["low"] - (current["atr"] * 0.6)
                    tp_distance = (c_price - sl) * 2
                    final_tp = c_price + tp_distance
                    return {
                        "symbol": symbol,
                        "signal": "LONG",
                        "entry_price": c_price,
                ...
                if current["high"] >= brk["price"] * 0.999 and c_price < brk["price"]:
                    sl = current["high"] + (current["atr"] * 0.6)
                    tp_distance = (sl - c_price) * 2
                    final_tp = c_price - tp_distance
                    return {
                        "symbol": symbol,
                        "signal": "SHORT",
