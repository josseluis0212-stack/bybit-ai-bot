...
class GridAnalyzer:
    ...
    def analyze_grid(self, symbol):
        ...
        if es_brusco:
            # print(f"Grid skip {symbol}: BTC brusco")
            return
        # 1. Análisis Macro (Tendencia 1D)
        klines_1d = self.client.get_kline(symbol=symbol, interval="D", limit=500) # Más datos para EMA 200
        ...
        # Usamos EMA 200 si existe, si no, EMA 50 (mid) como fallback para monedas nuevas
        trend_baseline = last_1d['ema_200'] if not pd.isna(last_1d['ema_200']) else last_1d['ema_mid']
        
        # Si ni siquiera hay EMA 50, usamos la primera EMA lenta disponible o simplemente permitimos
        ...
        if not is_long_1d and not is_short_1d:
            # print(f"🔍 [GRID SKIP] {symbol}: No hay alineación básica (Close vs EMA 200)")
            return
        ...
        if is_long_1d and last_1h['close'] < trend_baseline_1h: 
            # print(f"🔍 [GRID SKIP] {symbol}: 1H por debajo de EMA 200")
            return
        if is_short_1d and last_1h['close'] > trend_baseline_1h: 
            # print(f"🔍 [GRID SKIP] {symbol}: 1H por encima de EMA 200")
            return
        ...
        # Filtro RSI Momentum (Relajado a 45 para Long y 55 para Short)
        if is_long_1d and last_15m['rsi'] < 45: return 
        if is_short_1d and last_15m['rsi'] > 55: return 

        # Filtro Volumen (Permisivo: > 80% del promedio)
        avg_volume = df_15m['volume'].tail(20).mean()
        if last_15m['volume'] < (avg_volume * 0.8): return

        # Filtro ATR (Evitar lateralización)
        # Bajamos el umbral de volatilidad mínima para ser un poco más permisivos: 0.3%
